...
Pages.prototype.fetch = function (pagen, all, re) {
  re = re || /.*/;
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    gitmech.ls("*.md", function (err, list) {
      ...
      listWithData = listWithData.filter(function (page){
        return re.test(path.basename(page.name));
      });
      ...
      if (!all)
      {
        ...
        slice = listWithData.slice(offset, offset + itemsPerPage);
      }
      else
      {
        slice = listWithData;
